...
parfor i = 1:nChan %will do filtering and re-reference in this loop
    tic
    if flag==2
    sevfilename=[first_part_file num2str(2) '_ch' num2str(i) '.sev']; %#ok<PFTUS>
    elseif flag==3
    ...
    while 1
        ...
        if chunkInd <= nChunksTotal
            ...
            for i = 1:nChan
                ...
                if chunkInd == 1
                    if flag==2
                    sevfilename=[d(1).name(1:in_block+2) num2str(2) '_ch' num2str(i) '.sev'];
                    elseif flag==3
                        sevfilename=[d(1).name(1:in_block+2) num2str(2) '_ch' num2str(i+64) '.sev'];
                    elseif flag==5
                        sevfilename=[d(1).name(1:in_block+2) num2str(5) '_ch' num2str(i) '.sev'];
                    end
                    chfid(i) = fopen(fullfile(folder_path,sevfilename),'r'); %#ok<AGROW>
                end
                % read the data
